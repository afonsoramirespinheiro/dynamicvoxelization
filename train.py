...
def example_convert_to_torch(example, dtype=torch.float32,
    ...
    example_torch = {}
    float_names = [
        "anchors", "reg_targets", "reg_weights", "bev_map", "rect",
        "Trv2c", "P2"
    ]
    for k, v in example.items():
        if k in float_names:
            example_torch[k] = torch.as_tensor(v, dtype=dtype, device=device)
        elif k in ["coordinates", "labels","pillar_coor_points"]:
            example_torch[k] = torch.as_tensor(
                v, dtype=torch.int32, device=device)
        elif k in ["anchors_mask"]:
            example_torch[k] = torch.as_tensor(
                v, dtype=torch.uint8, device=device)
        elif k in ["indexes_vect","num_points_voxel"]:
            example_torch[k] = torch.as_tensor(
                v, dtype=torch.long, device=device)
        elif k in ["voxels"]:
            example_torch[k] = torch.as_tensor(
                v, dtype=torch.float, device=device)
        else:
            example_torch[k] = v
...
def train(config_path,
    ...
    print("Remaining_steps",remain_steps)
    t = time.time()
    ckpt_start_time = t
        ...
        for _ in range(total_loop):
            ...
            for step in range(steps):
                ...
                if global_step % display_step == 0:
                    ...
                    metrics["num_vox"] = int(example_torch["coordinates"].shape[0])
                    ...
                    metrics["mem_usage"] = torch.cuda.max_memory_allocated()
