...
def merge_second_batch(batch_list, _unused=False):
    #When batch_size = 2, len(batch_list) = 2.
    ...
    for key, elems in example_merged.items():
        if key in [
                'voxels',
                'num_gt', 'gt_boxes', 'voxel_labels',
                'match_indices','pillar_coor_points','num_points_voxel'
        ]:
        ...
        elif key == 'indexes_vect':
            indexes_vect =[]
            a = 0
            for i in elems:
                if a != 0:
                    max_p = np.asarray(indexes_vect[a-1]).max(axis=0)
                    indexes_vect.append((np.asarray(i)+max_p+1).tolist())
                else:
                    indexes_vect.append(i)
                a = a + 1
            
            ret[key] = np.concatenate(indexes_vect,axis=0)
        else:
...
def prep_pointcloud(input_dict,
    ...
    voxels, coordinates, indexes_vect, num_points_voxel,pillar_coor_points = voxel_generator.generate(
        points, max_voxels)
    example = {
        'voxels': voxels,
        'indexes_vect': indexes_vect,
        'coordinates': coordinates,
        "num_voxels": np.array([len(coordinates)], dtype=np.int64),
        "num_points_voxel": num_points_voxel,
        'pillar_coor_points': pillar_coor_points
    }
